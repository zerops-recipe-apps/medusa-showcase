...
- Runtime base: `nodejs@24` (SSR — not `static`)
...
- Prod start: `yarn start` (Zerops: `./node_modules/.bin/next start -p 8000`). If the process still has an unresolved `${medusa_CHANNEL_PUBLISHABLE_KEY}`, Node instrumentation exits so the supervisor respawns with the resolved `pk_` key.
...
- SDK: [`src/lib/config.ts`](src/lib/config.ts) accepts `NEXT_PUBLIC_MEDUSA_BACKEND_URL` or `MEDUSA_BACKEND_URL`. Region proxy ([`src/proxy.ts`](src/proxy.ts)) needs a reachable Medusa + publishable key at runtime. Read the key via [`getMedusaPublishableKey()`](src/lib/util/env.ts) (`MEDUSA_PUBLISHABLE_KEY` first; reject empty / `${…}` placeholders) so Next 16 cannot inline an empty build-time `NEXT_PUBLIC_*` and we never send an unresolved sibling ref. After Medusa seed writes `CHANNEL_PUBLISHABLE_KEY`, Zerops updates the container store but the running process keeps boot-time env — [`src/instrumentation.ts`](src/instrumentation.ts) exits so the supervisor starts a new process; medusa init also POSTs [`/api/internal/reload-env`](src/app/api/internal/reload-env/route.ts) (project `RELOAD_SECRET`; local dev may use `REVALIDATE_SECRET`) to exit immediately.
